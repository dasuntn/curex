...
const CountryItem: React.FC<Props> = ({ country }: Props) => {
  ...
  return (
    <TouchableOpacity
      ...
      <Flag code={country.alpha2Code} size={64} />
      <View style={countryListStyles.detail}>
        <Text numberOfLines={2} style={countryListStyles.nameText}>
          {country.name}
        </Text>
        <Text style={countryListStyles.capitalText}>{`Capital: ${country.capital}`}</Text>
        <Text style={countryListStyles.otherText}>{`Population: ${country.population}`}</Text>
        <Text
          style={countryListStyles.otherText}
          numberOfLines={2}>{`Currency: ${availableCurrencies}`}</Text>
      </View>
      <View style={countryListStyles.arrowIcon}>
        <Ionicons name="chevron-forward-outline" size={24} color={colors.darkGary} />
      </View>
    </TouchableOpacity>
  );
};
